...
public class FileSpanExporterTest extends OpenSearchTestCase {
    ...
    public void testOutputFormat() {
        ...
        Mockito.when(mockedSpanData.getInstrumentationScopeInfo()).thenReturn(InstrumentationScopeInfo.create("instrumentation-library-name", "1.0.0", null));
    ...
    private String getExpectedLogMessage(String name, String traceId, String spanId, String parentSpanId, AttributesMap attributesMap) {
        return String.format("'%s'\t%s\t%s\t%s\t%s\t%d\t%d\t[%s:%s:%s]\t%s",name, traceId, spanId, parentSpanId, "INTERNAL", 0, 0, "tracer", "instrumentation-library-name", "1.0.0", attributesMap);
    }
