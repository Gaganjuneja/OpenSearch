...
public class TracerUtils {
    ...
    public static void addTracerContextToHeader(Map<String, String> requestHeaders, Map<String, Object> transientHeaders) {
        ...
        if (transientHeaders != null && transientHeaders.containsKey(DefaultTracer.CURRENT_SPAN)) {
            SpanHolder spanHolder = (SpanHolder) transientHeaders.get(DefaultTracer.CURRENT_SPAN);
            Span currentSpan = spanHolder.getSpan();
            OSSpan osSpan = getLastOSSpanInChain(currentSpan);
            OTelMain.getOpenTelemetry().getPropagators().getTextMapPropagator()
    ...
    public static Context extractTracerContextFromHeader(Map<String, String> headers) {
        ...
        return OTelMain.getOpenTelemetry().getPropagators().getTextMapPropagator()
